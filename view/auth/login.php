<dialog id="captcha-dialog">
    <div class="w-[425px] p-6 fixed flex flex-col gap-6 items-center justify-center z-50 left-1/2 top-1/2 -translate-1/2 rounded-md bg-ms-white dark:bg-ms-black border-1 border-ms-grey dark:border-ms-white">
        ...
        <!-- <svg class="fill-ms-<?=$mood["color"]?> stroke-0 " width="100" height="100" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><circle cx="50" cy="50" r="40"/></svg>
        
        <svg class="fill-ms-<?=$mood["color"]?> stroke-0" width="100" height="100" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><rect width="80" height="80" x="10" y="10" /></svg>

        <svg class="fill-ms-<?=$mood["color"]?> stroke-0" width="100" height="100" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><polygon points="50,10 90,90 10,90" /></svg> -->
...
<main>
    ...
    <form method="POST" action="/auth/login/store/" class="flex flex-col justify-center items-center gap-6 h-svh">
        ...
        <div class="max-w-lg w-full flex flex-col gap-4 mx-4">
            <label for="captcha" class="flex items-center justify-between gap-6 border border-ms-<?=$mood['color']?> bg-ms-<?=$mood['color']?> px-8 py-4 rounded-md">
                ...
                <input type="checkbox" id="captcha-validate" class="w-6 h-6 border-transparent focus:ring-2 focus:ring-ms-<?=$mood['color']?> outline-hidden rounded-b-full hidden">
