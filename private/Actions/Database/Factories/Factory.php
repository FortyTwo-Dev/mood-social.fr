...
function CreateAllTables(PDO $conn): bool {
    try {
        ...
        CreateBadgeTable($conn);
        CreateUserBadgeTable($conn);
        return true;
...
function DropAllTables(PDO $conn) {
    $conn->prepare('DROP TABLE IF EXISTS USER_CUSTOM, USER_EXCHANGE_REACTION, USER_MESSAGE_REACTION, MESSAGE_LIKES, EVENT_LIKES, USER_MOOD, USER_EVENT, USER_TALK, FRIENDS, FOLLOWERS, EXCHANGES, LOGS, NEWSLETTERS, CUSTOMS, CAPTCHAS, ROLE_RIGHT, MESSAGES, EVENTS, MOODS, REACTIONS, USERS, TALKS, SUBSCRIPTIONS, ROLES, RIGHTS, USER_BADGE, BADGES;')->execute();
}
