    ...
    <ul class="w-full mt-6 flex flex-col gap-6">
        ...
        <li class="w-full py-2 px-4 text-2xl font-medium <?php if (str_starts_with($_SERVER['REQUEST_URI'], '/dashboard/users/')): ?> bg-ms-blue text-ms-white <?php else: ?> hover:bg-ms-blue/20 <?php endif; ?>">
            <a href="/dashboard/users/" class="flex items-center gap-4">
                <svg class="stroke-2 <?php if (str_starts_with($_SERVER['REQUEST_URI'], '/dashboard/users/')): ?> stroke-ms-white <?php else: ?> stroke-ms-black <?php endif; ?>" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                <p>Utilisateurs</p>
            </a>
        </li>
